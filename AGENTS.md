...
Pushing to GitHub is source control only. GitHub Actions deployment is intentionally disabled, and a push does not update the running production build. After completing, verifying, committing, and pushing each requested change, deploy it automatically unless the user explicitly says not to deploy. Run `./scripts/deploy-production.sh` from the repository root. That command requires a clean tree whose current commit is already on `origin`; it builds in an isolated worktree, applies migrations, swaps the standalone build, restarts only `website.service`, and performs health checks with rollback on local failure. Do not bypass it or invoke its privileged restart helper directly. Avoid changing Nginx, certificates, the database, or unrelated services unless the task requires it.

Do not block the conversation waiting for deployment to finish. Once the required deployment has been started, report that it is in progress and continue with the next user request or other useful scoped work. Keep the deployment output or process handle available, check it before claiming the deployment succeeded, and report any failure or rollback clearly. Never start a duplicate deployment while one is already running.
